...
const statusConfig = {
  approved: {
    title: 'Approved',
    description: "Congratulations! Your business registration has been approved. You can now access your wholesale account.",
  ...
  pending: {
    title: 'Pending',
    description: "Your registration is currently under review. We'll contact you within 1-2 business days.",
  ...
  rejected: {
    title: 'Rejected',
    description: "Unfortunately, your registration was not approved. Please contact support for more information.",
...
export const Register: React.FC = () => {
  ...
  // New state: approval status and loading flag
  ...
  // Prefill form if user/profile available
  useEffect(() => {
    if (user && profile) {
      setFormData(prev => ({
        ...prev,
  ...
  // Fetch approval_status for current user if exists
  useEffect(() => {
  ...
  if (loadingStatus) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-ethiopian-gold" />
      </div>
    );
  }

if (approvalStatus) {
  const status = statusConfig[approvalStatus.toLowerCase()] || {
    title: approvalStatus,
    description: "Your registration status is currently unavailable. Please contact support.",
    icon: <DefaultAlertCircle className="w-10 h-10 text-gray-500" />,
    color: 'gray',
    bgColor: 'bg-gray-50',
  ...
  return (
    <div className="min-h-screen flex items-start justify-center px-6 py-20 bg-gray-50">
      <Card
        className={`max-w-md w-full shadow-lg border ${status.borderColor} ${status.bgColor}`}
      >
        <CardHeader className="flex items-center space-x-4 p-6 border-b border-gray-200">
          <div>{status.icon}</div>
          <CardTitle className={`text-2xl font-semibold text-${status.color}-700`}>
            Registration Status: {status.title}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-6">
          <p className={`text-${status.color}-800 text-lg leading-relaxed`}>
            {status.description}
          </p>
          <p className="mt-6 text-sm text-gray-600">
            Thank you for registering. If you have any questions, please contact <a href="mailto:support@example.com" className="underline text-blue-600 hover:text-blue-800">support</a>.
          </p>
        </CardContent>
      </Card>
    </div>
            ...
            <form onSubmit={handleBusinessSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  readOnly
                  disabled
                  className="cursor-not-allowed border-ethiopian-gold/30 bg-gray-100"
                />
              </div>

              {/* Tax ID */}
              <div className="space-y-2">
                <Label htmlFor="taxId">Tax ID (EIN, SSN, or ITIN) *</Label>
                <Input
                  id="taxId"
                  name="taxId"
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`border-2 border-dashed rounded-md p-4 w-full cursor-pointer
                    border-ethiopian-gold/50
                    hover:border-ethiopian-gold focus:border-ethiopian-gold
                    ${
              ...
              <Button
                type="submit"
                className="bg-ethiopian-gold hover:bg-ethiopian-brown text-white w-full"
